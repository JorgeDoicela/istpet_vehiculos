...
const VehicleCard = ({ vehiculo, isSelected, isSuggested, onSelect }) => {
    ...
    return (
        <div
            ...
            className={`
                group relative p-2 rounded-2xl border-2 transition-all duration-500 cursor-pointer overflow-hidden 
            ...
            `}
        >
            {/* Badge Sugerido SIGAFI - Ultra Compacto */}
            {isSuggested && !isSelected && (
                <div className="absolute top-1 left-1.5 bg-emerald-500 text-white px-1 py-0.5 rounded-full text-[4px] font-black uppercase tracking-widest shadow-sm animate-pulse z-10">
                    Sugerido
                </div>
            )}

            {/* Icono de Vehículo Ultra Compacto */}
            ...
            <div className="text-center relative">
                ...
                <p className={`text-[5px] lg:text-[7px] font-black uppercase tracking-[0.2em] leading-none transition-colors ${isSelected ? 'text-[var(--apple-primary)]/70' : 'text-[var(--apple-text-sub)]'}`}>
                    {vehiculo.marca || 'UNIDAD'}
                </p>
            </div>

            {/* Check de Selección Ultra Compacto */}
            {isSelected && (
                <div className="absolute top-1.5 right-1.5 h-3 w-3 bg-[var(--apple-primary)] rounded-full flex items-center justify-center text-white scale-110 animate-apple-in shadow-sm">
