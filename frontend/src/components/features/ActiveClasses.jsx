    ...
    <div className="apple-card space-y-10 p-10 min-h-[500px] flex flex-col">
      ...
      <div className="flex-1 space-y-6">
        {classes.length === 0 ? (
          ...
          classes.map((c) => (
            <div key={c.idRegistro} className="group p-6 bg-white/40 rounded-3xl hover:bg-white transition-all duration-700 border border-transparent hover:border-white hover:shadow-xl hover:-translate-y-1">
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h4 className="text-sm font-black uppercase tracking-tighter text-slate-900">{c.estudiante}</h4>
                  <p className="text-[9px] font-black uppercase text-slate-400 tracking-widest">{c.cedula}</p>
                </div>
                <div className="text-right">
                  <p className="text-[14px] font-black tabular-nums text-slate-800">{new Date(c.salida).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
              ...
              <div className="flex items-center gap-4 p-3 bg-white/60 rounded-2xl">
                 <div className="w-8 h-8 rounded-xl bg-[var(--apple-primary)] flex items-center justify-center text-white shadow-lg">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M8.25 18.75a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m3 0h6m-9 0H3.375a1.125 1.125 0 01-1.125-1.125V14.25m17.25 4.5a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m3 0h1.125c.621 0 1.129-.504 1.129-1.125V14.25M7.5 14.25v-3.375c0-.621.504-1.125 1.125-1.125h9.192c.465 0 .867.285 1.03.681l1.43 3.494c.041.1.063.208.063.317v1.125m-15.5 0h15.5" />
                    </svg>
                 </div>
                 <div>
                    <h5 className="text-[10px] font-black text-slate-800 tracking-tighter uppercase">{c.placa}</h5>
                    <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">Unidad #{c.numeroVehiculo}</p>
                 </div>
              </div>
            </div>
          ))
      ...
      <div className="mt-8 p-6 bg-slate-900 rounded-[2rem] text-white">
          ...
          <div className="flex justify-between items-end">
              <span className="text-3xl font-black text-white">{classes.length}</span>
