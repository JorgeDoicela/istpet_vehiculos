    ...
    <div className="apple-card space-y-6 lg:space-y-8 p-4 lg:p-8 min-h-[400px] flex flex-col transition-all">
      ...
      <div className="flex-1 space-y-3 lg:space-y-4 overflow-y-auto max-h-[600px] pr-2 custom-scrollbar">
        {safeClasses.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2 gap-3">
            {safeClasses.map((c) => (
              <div key={c.id_Registro || c.id_registro || Math.random()} className="group p-3 lg:p-4 bg-[var(--apple-bg)] rounded-2xl transition-all duration-500 border border-[var(--apple-border)] hover:border-[var(--apple-primary)]/40 hover:shadow-lg">
                <div className="flex justify-between items-start mb-3">
                  ...
                  <div className="text-right ml-2">
                     <p className="text-[11px] lg:text-[13px] font-black tabular-nums text-[var(--apple-primary)]">
                ...
                <div className="flex items-center gap-3 p-2 bg-[var(--apple-card)] rounded-xl border border-[var(--apple-border)]">
                   <div className="w-7 h-7 rounded-lg bg-[var(--apple-primary)] flex items-center justify-center text-white shadow-md">
      ...
      <div className="mt-6 p-4 lg:p-5 bg-[var(--istpet-navy)] rounded-2xl text-white shadow-lg relative overflow-hidden group">
          ...
          <div className="flex justify-between items-center relative z-10">
              ...
              <span className="text-[8px] font-black uppercase text-blue-400 tracking-widest bg-blue-500/10 px-2 py-1 rounded-md border border-blue-500/20">Sync OK</span>
          </div>
      </div>
    </div>
  );
